...
export class AppointmentController {
    ...
    @PostMapping('/block', UserAccessLevel.Authenticated)
    async block(req: Request, res: Response): Promise<void> {
        try {
            ...
            const request = new BlockAppointmentDTO(body);
            const dtoValidation = await validate(request);
            if (dtoValidation && dtoValidation.length > 0) {
                console.log(dtoValidation);
                const errors = parseValidationErrors(dtoValidation);
            ...
            if (isValidToken) {
                res.status(200).json({
                    status: 'OK',
                    data: await this.appointmentService.block(token, request),
                });
            }
        } catch (error) {
            if (error.name === 'TokenExpiredError') {
